...
# data_path = "/projects/MINDLAB2015_MR-YoungAddiction/scratch/MJ/IB_analysis/data"
...
for condition in condtions:
    data_file = "subject_012_%s.csv" % condition
    data = pd.read_csv(data_file, delimiter=";")

    if (condition == "singlePress") | (condition == "actionPress1"):
        data["recalc_1"] = (data["ansAngle"] < 90) & (data["pressAngle"] > 270)
        data["recalc_2"] = (data["ansAngle"] > 270) & (data["pressAngle"] < 90)
    ...
    if (condition == "singleTone1") or (condition == "actionTone1"):
        data["recalc_1"] = (data["ansAngle"] < 90) & (data["toneAngle"] > 270)
        data["recalc_2"] = (data["ansAngle"] > 270) & (data["toneAngle"] < 90)
        for index, row in data.iterrows():
            if row.recalc_1:
                data.ansAngle.ix[index] = row.ansAngle + 360
