...
clear all
close all
cd '/scratch4/MINDLAB2015_MR-YoungAddiction/MJ/IB_analysis'
...
for subject = 1:length(all_subjects);
    ...
    for condition = 1:length(conditionTypes)
        ...
        if condition == 1 % = actionPressHand
            [~,~,data] = xlsread(['/projects/MINDLAB2015_MR-YoungAddiction/scratch/MJ/IB_analysis/data/subject_',num2str(all_subjects(subject), '%03d'),'_',conditionType,'.xlsx']);
        ...
        if condition == 2 % = singlePressHand
            [~,~,data] = xlsread(['/projects/MINDLAB2015_MR-YoungAddiction/scratch/MJ/IB_analysis/data/subject_',num2str(all_subjects(subject), '%03d'),'_',conditionType,'.csv']);
            data.data = data(2:end,:);
